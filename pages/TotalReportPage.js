...
const TotalReportPage = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const resp = await fetch('http://220.225.104.138:3003/api/TotalReportGenerate', {
                method: 'GET',
            })
    ...
    return (
        <React.Fragment>
            ...
            <ToolkitProvider
                ...
                data={tableValue}
                columns={columns}
                exportCSV={{ onlyExportFiltered: true, exportAll: false, fileName: 'TotalReport' + date + '.csv' }}
                search
            >{props => (
                <div>
