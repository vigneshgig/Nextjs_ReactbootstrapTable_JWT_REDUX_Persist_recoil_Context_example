...
const Table = ({ selectedTopics , user}) => {
    ...
    },
    {
        ...
        validator: (newValue, row, column) => {
            ...
            if (json.time0 === '00:00:00' || json.time0 === '00:00'|| !regex.test(json.time0)) {
                return {
                    valid: false,
    ...
    useEffect(() => {
        const fetchData = async () => {
            const resp = await fetch('http://220.225.104.138:3003/api/GetTopics', {
                method: 'POST',
                headers: {
            ...
            if (json.success === 'Pass') {
                const json_tmp = json.result.map(x => {
                    x.Starting = JSON.stringify({time0:'00:00:00',time1:'00:00:00',time2:'00:00:00',time3:'00:00:00'})
                    x.Ending = JSON.stringify({time0:'00:00:00',time1:'00:00:00',time2:'00:00:00',time3:'00:00:00'})
                    return x
                })
                await setdefaultselected(json.result.map((x) => {
                    return x.id
                }))
    ...
    const onSubmitHandle = async (event) => {
        ...
        if (tableValue.length === 0 || check.length === 0) {
            ...
            for (let i = 0; i < tableValue_tmp.length; i++) {
                    ...
                    if (tmp_start.time0 === '00:00:00' || tmp_start.time0 === '00:00' || tmp_end.time0 === '00:00:00' || tmp_end.time0 === '00:00') {
                        checkValidation = false;
                        document.getElementById('tableupdate').style.background = 'red';
                        await setMessage('starting and ending time is not filled completely please it fillout and retry again....')
                        break
                    }
        ...
        if (checkValidation) {
            ...
            const res = await fetch('http://220.225.104.138:3003/api/SetTagTopics', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (json.Updated) {
                ...
                const get_topics = await fetch('http://220.225.104.138:3003/api/GetTopics', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (get_json.success === 'Pass') {
                    ...
                    document.getElementById('tableupdate').style.background = 'green';
                    await setdefaultselected(get_json.result.map((x) => {
                        return x.id
                    }))
                    await setTableValue(json_tmp)
                    await dispatch(ActionCountTable(json.count))
    ...
    return (
        <React.Fragment>
            <BootstrapTable keyField='id' data={tableValue} columns={columns} cellEdit={cellEditFactory({ mode: 'click', blurToSave: true, })} noDataIndication='Table is Empty' selectRow={selectRow} ref={n => setSelectedRow(n)} />
